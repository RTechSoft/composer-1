...
export class ConnectionProfileComponent {
    ...
    openAddCertificateModal(index, type) {
        ...
        if (type === 'orderers' || type === 'peers') {
            ...
            if (!has(object, 'grpcOptions.sslTargetNameOverride')) {
        ...
        return modelRef.result
            .then((result) => {
                if (type === 'orderers' || type === 'peers' || type === 'ca') {
                  if (result) {
    ...
    isNumber(value) {
      ...
      return value.match(/^(\d+|\d+\.\d+)$/); // USING REGEX AS ISNAN ALLOWS NUMBERS WITH LETTERS IN LIKE 1e1000
    }
